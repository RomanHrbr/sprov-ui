...
[[ -d "/proc/vz" ]] && echo -e "${red}Error:${plain} Su VPS se basa en OpenVZ, que no es compatible." && exit 1
...
display_menu(){
    ...
    local hint
    local vname
    local prompt="which ${soft} Seleccionaría ${default_prompt}: "
    ...
    do
        ...
        if ! is_digit "$pick"; then
            prompt="Error de entrada, ingrese un número"
            continue
        fi

        if [[ "$pick" -lt 1 || "$pick" -gt ${#arr[@]} ]]; then
            prompt="Error de entrada, ingrese un número entre 1 y ${#arr[@]}: "
            continue
        fi
...
get_latest_version() {
    ...
    [ ${#latest_version[@]} -eq 0 ] && echo -e "${red}Error:${plain} No se pudo obtener la última versión del kernel." && exit 1
    ...
    [ -z ${deb_name} ] && echo -e "${red}Error:${plain} Error al obtener el nombre del paquete binario del kernel de Linux, tal vez la compilación del kernel haya fallado. Por favor, elija otra e inténtelo de nuevo." && exit 1
}
...
install_elrepo() {
    ...
    if centosversion 5; then
        echo -e "${red}Error:${plain} no es compatible con CentOS 5"
        exit 1
    fi
    ...
    if [ ! -f /etc/yum.repos.d/elrepo.repo ]; then
        echo -e "${red}Error:${plain} Error al instalar elrepo, por favor verifíquelo"
        exit 1
    fi
}
    ...
    if [[ x"${release}" == x"centos" ]]; then
        if centosversion 6; then
            if [ ! -f "/boot/grub/grub.conf" ]; then
                echo -e "${red}Error:${plain} /boot/grub/grub.conf no encontrado, verifíquelo."
                exit 1
            fi
            sed -i 's/^default=.*/default=0/g' /boot/grub/grub.conf
        elif centosversion 7; then
            if [ ! -f "/boot/grub2/grub.cfg" ]; then
                echo -e "${red}Error:${plain} /boot/grub2/grub.cfg no encontrado, verifíquelo."
                exit 1
            fi
            grub2-set-default 0
        fi
...
reboot_os() {
    echo
    echo -e "${green}Info:${plain} El sistema debe reiniciarse."
    read -p "Do you want to restart system? [y/n]" is_reboot
    if [[ ${is_reboot} == "y" || ${is_reboot} == "Y" ]]; then
        reboot
    else
        echo -e "${green}Info:${plain} Se ha cancelado el reinicio ..."
        exit 0
    fi
}
...
install_bbr() {
    ...
    if [ $? -eq 0 ]; then
        echo
        echo -e "${green}Info:${plain} TCP BBR ya se ha instalado. No hay nada que hacer ..."
        exit 0
    fi
    check_kernel_version
    if [ $? -eq 0 ]; then
        echo
        echo -e "${green}Info:${plain} Llanura su kernel versión 4.9 es mayor En el pasado, Directamente Configuración TCP BBR ..."
        sysctl_config
    ...
    if [[ x"${release}" == x"centos" ]]; then
        ...
        if centosversion 6; then
            ...
            if [ -f "${rpm_kernel_name}" ]; then
                ...
                echo -e "${red}Error:${plain} de descarga ${rpm_kernel_name} verifíquelo."
                exit 1
            fi
            if [ -f "${rpm_kernel_devel_name}" ]; then
                rpm -ivh ${rpm_kernel_devel_name}
            else
                echo -e "${red}Error:${plain} de descarga ${rpm_kernel_devel_name} verifíquelo."
                exit 1
            fi
            ...
            if [ $? -ne 0 ]; then
                echo -e "${red}Error:${plain} Falló la instalación del último kernel, verifíquelo."
                exit 1
            fi
        fi
    elif [[ x"${release}" == x"debian" || x"${release}" == x"ubuntu" ]]; then
        [[ ! -e "/usr/bin/wget" ]] && apt-get -y update && apt-get -y install wget
        echo -e "${green}Info:${plain} Obteniendo la última versión del kernel ..."
        get_latest_version
        if [ -n ${modules_deb_name} ]; then
            wget -c -t3 -T60 -O ${deb_kernel_modules_name} ${deb_kernel_modules_url}
            if [ $? -ne 0 ]; then
                echo -e "${red}Error:${plain} de descarga ${deb_kernel_modules_name} verifíquelo."
                exit 1
            fi
        fi
        wget -c -t3 -T60 -O ${deb_kernel_name} ${deb_kernel_url}
        if [ $? -ne 0 ]; then
            echo -e "${red}Error:${plain} de descarga ${deb_kernel_name} verifíquelo."
            exit 1
        fi
        ...
        echo -e "${red}Error:${plain} El sistema operativo no es compatible, cambie a CentOS/Debian/Ubuntu e intente nuevamente."
        exit 1
    fi
...
echo " Kernel  : $kern"
echo "----------------------------------------"
echo " Auto instalar el último kernel para TCP BBR"
